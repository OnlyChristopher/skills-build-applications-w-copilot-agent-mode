...
const Activities = () => {
  ...
  const filteredActivities = useMemo(() => {
    ...
    return activities.filter(activity => JSON.stringify(activity).toLowerCase().includes(lowered));
  }, [activities, query]);

  const columns = filteredActivities.length > 0 ? Object.keys(filteredActivities[0]) : [];
            ...
            <table className="table table-striped table-hover align-middle">
              ...
              <tbody>
                {filteredActivities.length === 0 ? (
                  <tr>
                    <td colSpan={Math.max(columns.length, 1)} className="text-muted">No activities available.</td>
                  </tr>
                ) : (
                  filteredActivities.map((activity, idx) => (
                    <tr key={activity.id || idx}>
                      {columns.map(column => (
                        <td key={column}>{formatValue(activity[column])}</td>
                      ))}
                    </tr>
                  ))
                )}
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <p className="mb-2">Displaying {filteredActivities.length} activities from the Octofit API.</p>
                  <p className="mb-0 text-muted">Use the filter to narrow results.</p>
                </div>
