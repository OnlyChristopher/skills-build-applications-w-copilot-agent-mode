...
const Workouts = () => {
  ...
  const filteredWorkouts = useMemo(() => {
    ...
    return workouts.filter(workout => JSON.stringify(workout).toLowerCase().includes(lowered));
  }, [workouts, query]);

  const columns = filteredWorkouts.length > 0 ? Object.keys(filteredWorkouts[0]) : [];
            ...
            <table className="table table-striped table-hover align-middle">
              ...
              <tbody>
                {filteredWorkouts.length === 0 ? (
                  <tr>
                    <td colSpan={Math.max(columns.length, 1)} className="text-muted">No workouts available.</td>
                  </tr>
                ) : (
                  filteredWorkouts.map((workout, idx) => (
                    <tr key={workout.id || idx}>
                      {columns.map(column => (
                        <td key={column}>{formatValue(workout[column])}</td>
                      ))}
                    </tr>
                  ))
                )}
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">About Workouts</h5>
                  <button type="button" className="btn-close" aria-label="Close" onClick={() => setShowModal(false)} />
                </div>
                <div className="modal-body">
                  <p className="mb-2">Displaying {filteredWorkouts.length} workouts from the Octofit API.</p>
                  <p className="mb-0 text-muted">Filter by workout type, intensity, or duration.</p>
                </div>
